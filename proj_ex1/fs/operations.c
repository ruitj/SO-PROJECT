...
int create(char *name, type nodeType){
	...
	split_parent_child_from_path(new_path,&parent_name_parent,&parent_name_child); // variaveis temporarias
	parent_child_number=lookup(parent_name_parent);
	pthread_rwlock_wrlock(&inode_table[parent_child_number].lock);
	...
	pthread_rwlock_unlock(&inode_table[parent_child_number].lock);
...
int lookup(char *name) {          
	...
	char delim[] = "/",*saveptr;
	int inode_index[INODE_TABLE_SIZE],i=0,j;
