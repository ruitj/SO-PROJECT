    ...
    while (fgets(line, sizeof(line)/sizeof(char), inputFile)) {
        ...
        switch (op) {
            case 'c':
                ...
                switch (arg2[0]) {
                    case 'f':
                        res = tfsCreate(&sockfd,line,&servlen,&serv_addr, 'f');
                        if (!res)
                        ...
                        break;
                    case 'd':
                        res = tfsCreate(&sockfd,line,&servlen,&serv_addr, 'd');
                        if (!res)
            ...
            case 'd':
                ...
                break;
            case 'p':
               res = tfsPrint(&sockfd,line,&servlen,&serv_addr);
                if (!res)
            ...
            case 'm':
                if(numTokens != 3)
                    errorParse();
                res = tfsMove(&sockfd,line,&servlen,&serv_addr);
                if (!res)
                  printf("Moved: %s to %s\n", arg1, arg2);
                else
...
int main(int argc, char* argv[]) { 
    ...
    if (tfsMount(serverName,&servlen,&serv_addr,&sockfd) == 0)
